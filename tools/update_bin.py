...
def download_and_extract(repo: str, name: str, version: str) -> None:
    ...
    dst_dir = Path("pyautd3_link_ethercrab/bin")
...
def should_update_dll(version: str) -> bool:
    if platform.system() == "Windows":
        if not Path("pyautd3_link_ethercrab/bin/autd3capi_link_ethercrab.dll").exists():
            return True
    elif platform.system() == "Darwin":
        if not Path("pyautd3_link_ethercrab/bin/libautd3capi_link_ethercrab.dylib").exists():
            return True
    elif not Path("pyautd3_link_ethercrab/bin/libautd3capi_link_ethercrab.so").exists():
        return True
...
def copy_dll() -> None:
    with Path("pyautd3_link_ethercrab/__init__.py").open() as f:
        content = f.read()
    ...
    download_and_extract("autd3-capi-link-ethercrab", "autd3-capi-link-ethercrab", version)
    shutil.copyfile("LICENSE", "pyautd3_link_ethercrab/LICENSE.txt")
    shutil.copyfile("ThirdPartyNotices.txt", "pyautd3_link_ethercrab/ThirdPartyNotices.txt")
